...
double avg_time_seqCS, avg_time_seqT, avg_time_mpiCS, avg_time_mpiT, avg_time_parallel, avg_speedup, avg_efficiency;  //avg values of analyses parameters 
...
int main(int argc, char **argv){
   ...
   if (argc < 2 || !checkPowerTwo(atoi(argv[1]))) {
    ...
    if(myrank == 0){
        ...
        printf("Num_Processes | Avg_MPI_Time | Avg_Speedup | Avg_Efficiency | Bandwidth \n");
    }
    ...
    if(myrank == 0){
        printf("%13d | %12f | %11f | %13.3f%% | %9.4f\n", size, avg_time_mpiCS, avg_speedup, avg_efficiency * 100, bandwidthMpi);
    }
...
void matTransposeMPI(double *M, double *T, int myrank, int size) {
    ...
    if (myrank == 0) {
        printf("%13d | %12f | %11f | %13.3f%% | %9.4f \n\n", size, avg_time_mpiT, avg_speedup, avg_efficiency * 100, bandwidthMpi);
    }
